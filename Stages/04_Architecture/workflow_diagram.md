# Vaquero Safety Inc — End-to-End Workflow Diagram (Mermaid.js)
**Source:** Phase 2 Workflow Specification | All 9 Stages  
**Render:** Copy the code block below into https://mermaid.live or any Mermaid-compatible renderer
...
flowchart TD
    ...
    %% STAGE 2 — INGESTION
    %% ============================================================
    subgraph S2["STAGE 2 — Safety Documentation Ingestion & Indexing"]
        direction TB
        ...
        S2_5[2.5 Retention label applied\nPurview enforcement\nno deletion before retention_end_date]
        S2_CEL[[CEL: document ingested\nclassifier_type + timestamp]]
    end
    ...
    subgraph S5["STAGE 5 — DocuSign Execution & Document Finalization"]
        ...
        S5_2[5.2 Envelope created\nSafety Manager: Routing Order 1\nExecutive: Routing Order 2\nExpiry: 30 days]
    ...
    subgraph S8["STAGE 8 — Chemical, SDS & Environmental Records [TEMPORARY]"]
        ...
        S8_chem[Chemical Inventory\nNew chemical added →\nSDS intake required before use]
        ...
        S8_env_b[EPEA immediate verbal report\ncall to Alberta Environment 1-800-222-6514\ncall confirmation → CEL]
        S8_well[Well Tracking\nAER docs stored\n/04-Site-Environmental/Well-Tracking/\nAdvisor confirms Petrinex submission]
        S8_note[[ALL STAGE 8 ITEMS TEMPORARY\nPENDING ENVIRONMENTAL/LEGAL REVIEW\nNo automated gov submissions built yet]]
    end
    ...
    %% ============================================================
    subgraph S9["STAGE 9 — Audit Trail, Reporting & COR Evidence"]
        direction TB
    ...
    class S1_3w,S2_2,S3_change,S3_reg,S4_3,S5_3,S5_change,S6_3,S7_2,S7_3,S8_env decision
...
| Color | Meaning |
|-------|---------|
| 🟠 Orange/Brown | **Human Required** — cannot be automated |
| 🔵 Blue | **Automated (Make.com)** |
| 🟢 Dark Green | **CEL Write** — compliance audit trail event |
| 🟣 Purple | **Decision Point** — branching logic |
| 💙 Steel Blue | **DocuSign** — envelope lifecycle nodes |
| 🔴 Dark Red | **Temporary / Flagged** — pending legal/environmental review |

---

## Four Circular Loops (Dashed Lines in Diagram)

1. **Core Compliance Loop**: Stage 5 activation → Stage 3 watches new expiry dates → Stage 4 alert → Stage 5 execution → loop
2. **SOP Update Loop**: Stage 3 detects regulatory change → Stage 6 propagates → Stage 4 per client → Stage 5 signs → Stage 3 monitors new version → loop
3. **Amendment Loop**: Stage 4 Amend → advisor revision → revised Stage 4 push → repeat until Approve
4. **Escalation Loop**: Stage 4 no-response → T+7 → T+14 → T+21 formal notice → loop terminates on response or documented non-compliance → Stage 9 CEL

---
*Render this file at https://mermaid.live — paste the code block contents only*

---

## Addendum — New Nodes (From: Adapting ChatGPT Model to Our Business)
*Added 2026-05-27. Insert these nodes and connections into the master diagram above at the indicated positions.*
...
flowchart TD
    ...
    %% ============================================================
    %% NEW: STAGE 3 EXTENSION — Contractor/Vendor Registry
    %% Insert after S3_cert node in Stage 3
    %% ============================================================
    S3_contractor[3.X Contractor/Vendor\nCredential Registry scan\nExtends Certification Tracker\nnew contractor_id type\nvendor_company + engagement_status]
    S3_contractor_alert{Contractor cert\nthreshold breached?}
    S3_contractor_cta[Alert → Safety Manager\nsame 90/60/30/7-day ladder\nas employee certifications]

    S3_cert --> S3_contractor --> S3_contractor_alert
    S3_contractor_alert -- Yes --> S3_contractor_cta --> S4_entry
    S3_contractor_alert -- No --> S3_CEL

    %% ============================================================
    %% NEW: STAGE 9 EXTENSION — COR Readiness Score
    %% Insert as parallel branch in Stage 9, fires nightly
    %% ============================================================
    S9_score[9.X COR Readiness Score\nNightly Make.com calculation\nInputs: cert expiry status +\nSOP currency + inspection rate +\nopen approvals + CEL density\nOutput: single score → Client Registry]
    S9_posture[9.X Compliance Posture Page\nPer-client SharePoint page\nLive data from existing lists\nNo new platform\nReplaces report-only touchpoint]

    S9_cel_agg --> S9_score --> S9_posture

    %% ============================================================
    %% NEW: STAGE 9 EXTENSION — Financial Risk Translation
    %% Insert into quarterly report generation
    %% ============================================================
    S9_risk[9.X Financial Risk Translation\nRule-based estimates only\nWCB premium impact\nCOR lapse penalty exposure\nInsurance eligibility risk\nOutput: dollar range in quarterly report]

    S9_score --> S9_risk --> S9_quarterly

    class S3_contractor,S3_contractor_cta,S9_score,S9_posture,S9_risk new
    class S3_contractor_alert decision
```

### Integration Notes for Master Diagram

| New Node | Connects From | Connects To | Stage |
|----------|--------------|-------------|-------|
| Contractor/Vendor Registry scan | `S3_cert` (daily scan) | `S4_entry` on threshold breach | S3 extension |
| COR Readiness Score | `S9_cel_agg` (nightly) | `S9_posture` → `S9_monthly` | S9 extension |
| Compliance Posture Page | `S9_score` | Passive output — no downstream trigger | S9 extension |
| Financial Risk Translation | `S9_score` | `S9_quarterly` report | S9 extension |

### Deferred Nodes — Do Not Add Until ADR Authorizes

| Node | Reason Deferred |
|------|----------------|
| Unified Cross-Industry Taxonomy | Blocked until all 4 compliance matrices at Production status |
| Client-Facing Read-Only Portal | Blocked until Compliance Posture Page (#38) is live and validated |
| ML Predictive Scoring | Deferred indefinitely — rule-based logic sufficient |
