...
## ADDENDUM — New Items (From: Adapting ChatGPT Model to Our Business)
*Added 2026-05-27 | Source: Strategic analysis session*

### CATEGORY 8 — NEW SHAREPOINT LISTS / FIELDS

| # | List / Field | Purpose | Stage |
|---|-------------|---------|-------|
| SP14 | **Contractor/Vendor Registry** (new list, extends Certification Tracker model) | Tracks subcontractors and vendors operating at client sites. Fields: `contractor_id`, `vendor_company`, `worker_name`, `certification_type`, `certifying_body`, `issue_date`, `expiry_date`, `engagement_status` (Active/Inactive/Suspended), `client_id`, `site_id`, `last_alert_sent`. | S3 extension |
| SP15 | **COR Readiness Score field on Client Registry** (new field on SP1) | Single numeric field populated nightly by Make.com scenario. Inputs are aggregated from Certification Tracker, SOP Registry, Asset Registry, Notification Log, and CEL. Powers Compliance Posture Page and financial risk estimates. | S9 |
| SP16 | **Financial Risk Estimate fields on Client Registry** (new fields on SP1) | Rule-based dollar exposure estimates: `wcb_premium_risk_range`, `cor_lapse_penalty_estimate`, `insurance_eligibility_flag`, `last_risk_calc_date`. Populated by same nightly Make.com scenario as COR Readiness Score. | S9 |

### CATEGORY 9 — NEW NOTIFICATION TEMPLATES

| # | Template Name | Trigger | Key Content | Stage |
|---|--------------|---------|-------------|-------|
| N19 | **Contractor Credential Expiry — 60 Day Action Request** | Contractor/Vendor Registry threshold | Contractor name, company, cert type, expiry date | S3 extension, S4 |
| N20 | **Contractor Credential Expiry — 30 Day Critical Alert** | Contractor/Vendor Registry threshold | Same as above + advisor alerted | S3 extension, S4 |
| N21 | **Contractor Credential Expired (T+0)** | Contractor status = Expired | Site access risk flag; Safety Manager and advisor alerted | S3 extension, S4 |
| N22 | **Financial Risk Summary — Quarterly Executive Report** (updated template) | Scheduled quarterly | Existing quarterly content + new section: compliance gap dollar exposure estimates, WCB premium impact range, COR lapse risk window | S9 |

### CATEGORY 10 — NEW INTERNAL OPERATIONAL DOCUMENTS

| # | Document | Purpose | Owner | Stage |
|---|----------|---------|-------|-------|
| O10 | **COR Readiness Score Methodology Document** | Documents the weighting logic for each input variable in the nightly score calculation. Required for client transparency, advisor override authority, and COR audit defensibility. Must be reviewed by compliance advisor before score goes live. | Compliance Advisor + Tech Team | S9 |
| O11 | **Financial Risk Translation Rules Document** | Documents the rule-based logic for each dollar exposure estimate (WCB formula inputs, COR penalty ranges, insurance eligibility criteria). Must be reviewed by Alberta labour lawyer and compliance advisor before surfacing estimates to clients. | Legal + Compliance Advisor | S9 |
| O12 | **Contractor/Vendor Onboarding Checklist** | Advisor-facing checklist for adding a new subcontractor to the Contractor Registry. Confirms: company name verified, credential copies received, engagement_status confirmed, client_id linked, initial alert ladder initialized. | Compliance Advisor | S3 extension |
| O13 | **Competitive Positioning Reference — Industrial OHS** | Internal document defining Vaquero's positioning against ISNetworld, Avetta, Veriforce/ComplyWorks, Cognibox. Includes the differentiator set (push + DocuSign + SharePoint + CEL), target buyer psychology ("worker hurt, paperwork wrong"), and displacement strategy for contractor registry feature vs. ISNetworld. Not a marketing document — for advisor and sales use only. | Compliance Advisor + Operations | GTM-Strategy |

---

## UPDATED SUMMARY: TOTAL DELIVERABLES

| Category | Count | Delta | Status |
|----------|-------|-------|--------|
| Legal Agreements | 4 | — | 🔴 2 must draft before any client onboarding |
| SharePoint Lists | 13 + 3 new fields/lists | +3 | 🟡 Schema finalization in PnP template |
| Document Libraries | 10 | — | 🟡 Part of PnP template build |
| DocuSign Templates | 9 | — | 🟡 Build after MSA legal draft complete |
| Notification Templates | 18 + 4 new | +4 | 🟡 Build with Make.com scenario pass |
| Forms | 7 | — | 🟡 2 live before first client |
| Internal Operational Docs | 9 + 4 new | +4 | 🟡 O11 requires legal review before score surfaces to clients |
| **TOTAL** | **78** | **+8** | |

*Addendum added 2026-05-27 | Source: "Adapting ChatGPT model to our business" strategic session*
