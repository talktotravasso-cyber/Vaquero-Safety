...
## TIER 6 — NEW BUILD ITEMS (From: Adapting ChatGPT Model to Our Business)
*Source: Strategic analysis session 2026-05-26. Five net-new capabilities identified. All reuse existing infrastructure — no new platforms required.*

| # | Stage | Item | Owner | Priority | Status |
|---|-------|------|-------|----------|--------|
| 37 | S9 | **COR Readiness Score** — Weighted nightly Make.com calculation. Inputs: Certification Tracker expiry status, SOP version currency, inspection completion rate, outstanding approval actions, CEL event density. Output: single numeric score field on SharePoint Client Registry record; surfaced in monthly executive report. No new infrastructure — reuses all existing lists. | Tech Team | 🔴 NOW — no new infra required | 🟡 NOT STARTED |
| 38 | S9 | **Compliance Posture SharePoint Page** — Per-client live data view rendering existing list data (Certification Tracker, open action items, SOP registry, inspection completion). Replaces report-only executive touchpoint. No new platform — embedded SharePoint web parts. Addresses likely early sales objection: "We need a dashboard." | Tech Team | 🔴 NOW — no new infra required | 🟡 NOT STARTED |
| 39 | S3/S7 | **Contractor/Vendor Credential Registry** — Extend Certification Tracker model to external subcontractors and vendors operating at client sites. Tracks worker-level credentials for non-employees. High value: direct competitive response to ISNetworld ($1,000+/vendor/year). Reuses 80% of existing architecture. Same alert ladder, same CEL write pattern, same DocuSign execution path. Requires: new `contractor_id` field type, `vendor_company` field, `engagement_status` (Active/Inactive/Suspended). | Tech Team + Compliance Advisor | 🟡 NEXT 90 DAYS | 🟡 NOT STARTED |
| 40 | S9 | **Financial Risk Translation Layer** — Rule-based estimates in quarterly executive report. Translate compliance gaps to dollar exposure: WCB premium impact from COR lapse, contractual penalty risk from MSA COR requirements, insurance eligibility risk. No ML required — rule-based sufficient for v1. Inputs: COR Readiness Score + open action items list + expiry counts. Output: dollar range estimates in quarterly report. Highest executive conversion lever identified. | Compliance Advisor (rules) + Tech Team (automation) | 🟡 NEXT 90 DAYS | 🟡 NOT STARTED |
| 41 | All | **Unified Cross-Industry Control Taxonomy** — After all 4 compliance matrices complete (O&G done; Construction, Logging, Trucking pending), normalize credentials and regulatory requirements into a single schema. Clients spanning industries get one Certification Tracker, not multiple. Prerequisite: all 4 matrices must reach Production status first. Do not build prematurely. | Compliance Advisor + Tech Team | 🔵 6–12 MONTHS | 🔴 BLOCKED — matrices incomplete |
| 42 | S9 | **Client-Facing Read-Only Portal** — SharePoint embedded view or lightweight web layer. Magic link or SSO. For executive on-demand access before board meetings or insurance renewals. Not a daily-use tool — supplementary to push-only model. Do not build until Compliance Posture Page (#38) is live and tested. | Tech Team | 🔵 6–12 MONTHS | 🔴 BLOCKED — #38 prerequisite |

---

## EXPLICITLY DEFERRED — Do Not Build Without ADR Entry

These were assessed and rejected during the 2026-05-26 strategic session. Do not re-open without a new Architecture Decision Record:

| Item | Reason |
|------|--------|
| Custom dashboard application | SharePoint adequate for 18+ months; significant engineering cost with no near-term value |
| Real-time SIEM integrations | Irrelevant to industrial SMB client profile |
| Multi-framework cyber overlay (SOC 2, ISO 27001) | Out of scope — Vaquero clients have no SOC 2 obligations |
| Predictive ML forecasting / scoring | Rule-based threshold logic = 80% of value at 5% of cost; revisit at 50+ clients |
| Daily-login client portal | Contradicts push-only model which is a core differentiator; do not build |

---
*Tier 6 added 2026-05-27 | Source: "Adapting ChatGPT model to our business" strategic session*
